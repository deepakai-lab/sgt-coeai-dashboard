    ...
    <>
      <section className="relative border-b border-border">
        ...
        <div className="container relative mx-auto px-4 pt-24 pb-20">
          <div className="eyebrow mb-6">Resources</div>
          <h1 className="text-4xl md:text-6xl font-medium tracking-tighter leading-[1.02] max-w-3xl">
            Guides, training,<br />
            <span className="text-muted-foreground">things we&apos;ve published.</span>
          </h1>
        ...
        {(!resources || resources.length === 0) ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 border-t border-l border-border">
            {resources.map((r: any) => {
              ...
              return (
                <div key={`${r.kind}-${r.id}`} className="border-r border-b border-border p-8 bg-card/30 hover:bg-card transition-colors group">
                  <div className="flex items-start gap-5">
                    <div className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border bg-background">
                      <Icon className="h-4 w-4 text-muted-foreground stroke-[1.5]" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 text-xs mb-2">
                        <span className="text-muted-foreground font-mono uppercase tracking-wider">{r.kind}</span>
                      ...
                      {r.file_url && (
                        <a href={r.file_url} target="_blank" rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mt-4 group-hover:gap-2 transition-all">
