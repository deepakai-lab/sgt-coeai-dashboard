      ...
      <section className="relative border-b border-border overflow-hidden">
        ...
        <div className="container relative mx-auto px-4 pt-12 pb-20 max-w-4xl">
          ...
          <div className="flex items-center gap-4 text-xs mb-8">
            <span className="num uppercase tracking-wider text-muted-foreground">{project.project_type}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            {project.public_description ? (
              <p className="whitespace-pre-wrap text-base md:text-lg leading-relaxed text-foreground/90">
                {project.public_description}
              </p>
            ) : (
...
function Row({ label, children, mono }: { label: string; children: React.ReactNode; mono?: boolean }) {
  return (
    <div className="flex justify-between items-center py-3 border-b border-border last:border-b-0">
      <span className="text-xs text-muted-foreground uppercase tracking-wider num">{label}</span>
      <span className={`text-sm ${mono ? 'num' : ''}`}>{children}</span>
    </div>
  )
}
