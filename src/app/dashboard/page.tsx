...
export default async function DashboardHome() {
  ...
  return (
    <>
      ...
      {/* Stat grid — bordered, monochrome, mono numerals */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 border-t border-l border-border mb-8">
              ...
              activity.slice(0, 6).map((a: any) => (
                <div key={a.id} className="flex items-start gap-3 text-xs">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-muted-foreground font-mono mt-0.5 text-[10px]">{formatDateTime(a.created_at)}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="divide-y divide-border">
            {recentMeetings.length === 0 ? (
              ...
              recentMeetings.map((m: any) => (
                <Link key={m.id} href={`/dashboard/meetings/${m.id}`} className="flex items-center justify-between gap-3 px-5 py-3 hover:bg-accent/40 transition-colors group">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm truncate group-hover:text-foreground">{m.title}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{m.meeting_type}</div>
                  </div>
                  <div className="text-xs text-muted-foreground font-mono shrink-0">{m.meeting_date}</div>
                </Link>
              ))
            )}
          </div>
...
function Stat({ label, value, sub, href, alert }: {
  ...
  return (
    <Link href={href} className="group border-r border-b border-border p-5 bg-card hover:bg-accent/40 transition-colors block">
      ...
      <div className="text-3xl num font-medium text-foreground">{value}</div>
